...
select * from [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].gender
*/
...
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Card_Holder] J002 ON J002.[Person_ID] = J001.[Client_ID]
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Card_Type] J003 ON J003.[Card_Type_ID] = J002.[Card_Type_ID]
----
...
(
	...
	From [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Personal_Contact] PC WITH(NOLOCK)
		Left outer Join [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Personal_Contact_Type] PCT WITH(NOLOCK) ON PCT.[Personal_Contact_Type_Code] = PC.[Personal_Contact_Type_Code] 
	Where 
		1=1
...
(
	...
	from [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Person] P  WITH(NOLOCK)
		Inner Join [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Title] T WITH(NOLOCK) on P.[Title_Code] = T.[Title_Code]

) J006 ON J006.[Person_ID] = J004.[Contact_ID]

LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Task_Schedule_Allocation] J007 WITH(NOLOCK) ON J007.[Client_ID] = J001.[Client_ID]
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Round_Allocation] J008 ON J008.[Schedule_Sequence_No] = J007.[Schedule_Sequence_No]
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Service_Provision_Position] J009 ON J009.[Service_Prov_Position_ID] = J008.[Service_Prov_Position_ID]
Left outer JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Position_Allocation] J010 ON J010.[Service_Prov_Position_ID] = J009.[Service_Prov_Position_ID]
...
--GOC stuff
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[FC_Client_Contract] J012 WITH(NOLOCK) ON J012.[Client_ID] = J001.[Client_ID]
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[FC_Client_Goal_of_Care] J013 ON J013.[Client_Contract_ID] = J012.[Client_Contract_ID]




--Organisation filtering Primary.
LEFT OUTER JOIN [APPSQL-3\COMCAREPROD].[comcareprod].[dbo].[Service_Delivery] J017 WITH(NOLOCK) ON J017.[Client_ID] = J001.[Client_ID]
