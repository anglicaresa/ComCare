...
select * FROM dbo.Scheduling_Activity
where
...
select * FROM dbo.Schedule_Allocation_Time
...
INNER JOIN dbo.Service_Delivery J003 ON J003.Client_ID = J002.Client_ID
...
(
	...
	FROM dbo.Service_Delivery SD 
		INNER JOIN dbo.Period_of_Residency PR ON PR.Person_ID = SD.Client_ID 
		INNER JOIN dbo.Address A ON A.Address_ID = PR.Address_ID 
		INNER JOIN dbo.Service_Provision SP ON A.Suburb_ID = SP.Suburb_ID AND SP.Service_Type_Code = SD.Service_Type_Code 
		INNER JOIN dbo.Organisation O ON SP.Centre_ID = O.Organisation_ID
    WHERE
		(PR.To_Date IS NULL) 
		AND (PR.Display_Indicator = 1)
)J004 ON J004.Client_ID = J002.Client_ID AND J004.Service_Type_Code = J003.Service_Type_Code
 
Inner Join dbo.Person_Current_Address_Phone J005 ON J005.Person_id = J002.Client_ID 
Inner Join dbo.Task_Type J006 ON J006.Task_Type_Code = J001.Schedule_Task_Type
