...
(
	...
	From [dbo].[Personal_Contact] PC WITH(NOLOCK)
		Left outer Join [dbo].[Personal_Contact_Type] PCT WITH(NOLOCK) ON PCT.[Personal_Contact_Type_Code] = PC.[Personal_Contact_Type_Code] 
	Where 
		1=1
...
(
	...
	from [dbo].[Person] P  WITH(NOLOCK)
		Inner Join [dbo].[Title] T WITH(NOLOCK) on P.[Title_Code] = T.[Title_Code]

) J006 ON J006.[Person_ID] = J004.[Contact_ID]

LEFT OUTER JOIN [dbo].[Task_Schedule_Allocation] J007 WITH(NOLOCK) ON J007.[Client_ID] = J001.[Client_ID]
...
--GOC stuff
LEFT OUTER JOIN [dbo].[FC_Client_Contract] J012 WITH(NOLOCK) ON J012.[Client_ID] = J001.[Client_ID]
...
LEFT OUTER JOIN [dbo].[Service_Delivery] J017 WITH(NOLOCK) ON J017.[Client_ID] = J001.[Client_ID]
...
(
	...
	from [dbo].Service_Delivery SD WITH(NOLOCK)
		join [dbo].Period_of_Residency PR WITH(NOLOCK) on PR.Person_ID = SD.Client_ID
