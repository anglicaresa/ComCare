...
(
	select distinct
		...
		,0 'AdjFix'
		,J033.WiA_Schedule_TimeKILL--Last
		,J033.SPPID
		,J001.Service_Prov_Position_ID
		,J033.Allocated_Task_ID 'Allocated_Task_ID_Wi'
		,J001.Allocated_Task_ID 'Allocated_Task_ID_As'
		------------------------new------------------------
		,iif(J033.SPPID != J001.Service_Prov_Position_ID,'forced','natural')'LinkType'
		,ROW_NUMBER()over
		(partition by J001.Client_ID, J001.Provider_ID, Cast (cast(J001.Visit_Date as date) as Datetime) + Cast (Cast (J001.Visit_Time as Time) as Datetime)
			order by case
				when J033.SPPID = J001.Service_Prov_Position_ID and Cast (cast(J001.Visit_Date as date) as Datetime) + Cast (Cast (J001.Visit_Time as Time) as Datetime) = J033.Schedule_Time then '1'
				when J033.SPPID = J001.Service_Prov_Position_ID then '2'
				when J033.SPPID != J001.Service_Prov_Position_ID and J033.SPPID != 0 then '3'
				when J033.SPPID != J001.Service_Prov_Position_ID and J033.SPPID = 0 then '4'
				else '4'
				end
		)'RN1'
	from 
	...
	(
		select
			...
			,Wi_A.Provider_ID --added
			,Wi_A.Activity_Date
			,Wi_A.Schedule_Task_Type --added
			,Wi_A.SPPID
	...
	--altered
		...
		and J033.Schedule_Task_Type = J001.Task_Type_Code
		/* 
...
where
	...
	and (t1.RN1 is null or t1.RN1 < 2)
---------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
---------------------------------------------------------------------------------------------------------------------------------
