/*
select * from dbo.Actual_Service where client_ID = 10016125 and Visit_Date = '2017-07-13 00:00:00.000'
...
Declare @Client_ID_ INT = 10014200
DECLARE @StartDate Date = '2017-09-15'
DECLARE @EndDate Date = '2017-09-15'
--print @StartDate
...
(
	...
	(
		...
		(
			select
				FC_T_SUB.FC_Account_ID 'FC_Account_ID'
				,FC_T_SUB.FC_Transaction_Type_ID 'FC_Transaction_Type_ID'
				,FC_T_SUB.Activity_Date 'Visit_Date'
				,FC_T_SUB.Transaction_Source 'Transaction_Source'
				,FC_T_SUB.Estimated_Amount 'Estimated_Amount'
				,FC_T_SUB.Actual_Amount 'Actual_Amount'
				,FC_T_SUB.Balance_After_Txn 'Balance_After_Txn'
				,FC_T_SUB.Source_Table 'Source_Table'
				,FC_T_SUB.Source_Record_Key 'Source_Record_Key'
				,FC_T_SUB.Comments 'Comments'
				,ROW_NUMBER() over
				(
					partition by FC_T_SUB.FC_Account_ID, FC_T_SUB.Activity_Date
					order by case
					when FC_T_SUB.Actual_Amount = 0.0 then 'zzzz'
					else FC_T_SUB.Actual_Amount
					end desc
				)'RN' --forces results to be ordered
			from dbo.FC_Transaction FC_T_SUB
			where 
				1=1
				and FC_T_SUB.FC_Transaction_Type_ID = 3
				and FC_T_SUB.Activity_Date between @StartDate and @EndDate
	...
	(
		...
		from (select * from dbo.wi_activity where Activity_Date between @StartDate and @EndDate) Wi_A
	...
	(
		select distinct
			TT.Task_Type_Code
			,TT.Service_Type_Code
			,SD.Client_ID 'Client_ID'
			,FC_FC.Description 'funderContract'
			,FC_FCM.Description 'CareModel'
			,SD.From_Date
			,iif(SD.To_Date is null,cast('2100-01-01'as date),SD.To_Date)'To_Date'
		...
		Left outer Join dbo.FC_Funding_Care_Model FC_FCM on FC_FCM.Funding_Care_Model_ID = FC_FC.Funding_Care_Model_ID
	)J088 on J088.Client_ID = J001.Client_ID and J088.Task_Type_Code = J001.Task_Type_Code and J001.Visit_Date between J088.From_Date and J088.To_Date
	...
	(
		...
		Where PR.Display_Indicator = 1
	) J006 ON J006.Client_ID = J001.Client_ID and J006.Service_Type_Code = J004.Service_Type_Code
...
(
	...
	(
		Select
			...
			,ROW_NUMBER() over -- sort by importance of 'covered' 'absent' and 'Un-Alocated'.
			(
	...
	)J088 on J088.Client_ID = J001.Client_ID and J088.Task_Type_Code = J001.Task_Type_Code and J001.WiA_Schedule_Time between J088.From_Date and J088.To_Date
