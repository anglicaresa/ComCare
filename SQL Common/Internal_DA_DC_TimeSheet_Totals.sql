--select * from dbo.Provider_Contract where Provider_ID = 10067951
--select * from dbo.Organisation
declare @Start_Date date = cast('2017-12-01' as date)
declare @End_Date date = cast('2018-01-14' as date)
DECLARE @OrgName AS Varchar(128) = 'Disabilities Children'

declare @ProvID int = 10098895
declare @TestMode bit = 1


--select * from dbo.Actual_Service J001 where J001.Provider_ID = @ProvID and cast(J001.Visit_Date as date) between @Start_Date and @End_Date
/*
...
Select distinct
	...
	,iif(J007.Activity_Type is null,'Task',J007.Activity_Type)'Activity_Type'
...
inner join dbo.Provider_Contract J006 on 
	J006.Organisation_ID = J001.Organisation_ID 
	and J001.Organisation_Name = @OrgName
	and @Start_Date < iif(J006.Effective_Date_To is null,Cast('2200-01-01' as date),J006.Effective_Date_To)
	and @End_Date > J006.Effective_Date_From
	
--this IS test field below>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
--and J006.Provider_ID = @ProvID --coment out!!!!!!!!!!
-->>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
--		,A_S.
	...
	where cast(A_S.Visit_Date as date) between @Start_Date and @End_Date
...
(
	...
	J007.Provider_ID = J006.Provider_ID
...
--Period totals
--/*
declare @ScheduleTotals Table (Provider_ID int,Client_Count int,Schedule_Duration decimal(10,2))
insert into @ScheduleTotals
Select distinct
	J001.Provider_ID
	,Count(J001.Client_ID)over(Partition by J001.Provider_ID)'Client_Count'
	,(sum(J001.Schedule_Duration)over(partition by J001.Provider_ID))/60.00'Schedule_Duration'
from 
(
	select
	* 
	...
	where 
		Wi_A.Provider_ID in (select distinct Provider_ID from @RawData)
		and (
			convert(date, Wi_A.Activity_Date) between @Start_Date and @End_Date
		)
) J001
...
select 
	* 
	...
	,(T1.TS_TaskLogged+TS_InternalTaskPaid) 'TotalPaidHours'
from
(
	select
--		JS001.Organisation_Name
		JS001.Provider_ID
		,JS001.Provider_Name
		,JS004.Client_Count 'Sch_ClientCount'
		,JS004.Schedule_Duration 'Sch_Duration'
		,Count (JS001.Client_ID) over(partition by JS001.Provider_ID) 'TS_ClientCount'
		,sum (iif (JS001.Activity_Type = 'Task' and JS001.Group_Activity_ID is null, JS001.Activity_Duration/60.0,0.0)) over(Partition by JS001.Provider_ID) 'TS_TaskLogged'
		...
		,sum (iif (JS001.Activity_Type = 'Task' and JS001.Group_Activity_ID is not null, JS001.Activity_Duration/60.0,0.0)) over(Partition by JS001.Provider_ID) 'TS_GroupClientTotalTime'
		...
		,sum (iif (JS001.Group_Activity_ID is null, JS001.AS_Duration/60.0,0.0)) over(Partition by JS001.Provider_ID) 'AS_TaskLogged'
		,sum (iif (JS001.Group_Activity_ID is not null, JS001.AS_Duration/60.0,0)) over(Partition by JS001.Provider_ID) 'AS_GroupClientTotalTime'
		,Count (JS001.ASCI_Client_ID) over(partition by JS001.Provider_ID) 'CI_ClientCount'
		,sum (iif (JS001.ASCI_UOM = 'Hour', JS001.ASCI_Unit,0.0)) over(Partition by JS001.Provider_ID) 'CI_Hours'
		,sum (iif (JS001.ASCI_UOM = 'Visit', JS001.ASCI_Unit,0.0)) over(Partition by JS001.Provider_ID) 'CI_Visit'
		,sum (iif (JS001.ASCI_UOM = 'Unit', JS001.ASCI_Unit,0.0)) over(Partition by JS001.Provider_ID) 'CI_Unit'
		,sum (iif(JS001.ASCI_Amount is null,0.0,JS001.ASCI_Amount)) over(Partition by JS001.Provider_ID) 'CI_TotalCharge'
		,JS003.Employee_No

	from @RawData JS001
	left outer join dbo.Provider JS003 on JS003.Provider_ID = JS001.Provider_ID
	left outer join @ScheduleTotals JS004 on JS004.Provider_ID = JS001.Provider_ID
...
--	T1.Organisation_Name
	...
	,(T1.TS_TaskLogged+TS_InternalTaskPaid)
